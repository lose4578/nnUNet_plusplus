...
import torch
import numpy as np
from nnunet.utilities.tensor_utilities import sum_tensor, my_sum_tensor
...
class nnUNetTrainer(NetworkTrainer):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        # TODO change loss to DC_and_FC_loss
        self.loss = DC_and_CE_loss({'batch_dice': False, 'smooth': 1e-5, 'do_bg': False, 'square': False}, {})
        # self.loss = DC_and_FC_loss({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False, 'square': False}, {})
        # self.loss = FocalLoss()
        ...
        if compute_global_dice:
            ...
            for l in all_labels:
                global_dice[int(l)] = float(
                    2 * global_tp[l] / (2 * global_tp[l] + global_fn[l] + global_fp[l]))
        ...
        with torch.no_grad():
            ...
            for c in range(1, num_classes):
                ...
                fn_hard[:, c - 1] = sum_tensor((output_seg != c).float()
                                               * (target == c).float(), axes=axes)
                # tp_hard[:, c - 1] = my_sum_tensor((output_seg == c).float() * (target == c).float())
                # fp_hard[:, c - 1] = my_sum_tensor((output_seg == c).float() * (target != c).float())
                # fn_hard[:, c - 1] = my_sum_tensor((output_seg != c).float() * (target == c).float())
    ...
    def finish_online_evaluation(self):
        ...
        for tp, fp, fn in zip(self.online_eval_tp, self.online_eval_fp, self.online_eval_fn):
            for i, j, k in zip(tp, fp, fn):
                kidney_list.append([i[0], j[0], k[0]])
                cancer_list.append([i[1], j[1], k[1]])

        kidney_dc_per = np.mean([i for i in [2 * i / (2 * i + j + k)
                                             for i, j, k in kidney_list] if not np.isnan(i)])
        cancer_dc_per = np.mean([i for i in [2 * i / (2 * i + j + k)
                                             for i, j, k in cancer_list] if not np.isnan(i)])

        # ipdb.set_trace()

        self.online_eval_tp = np.sum(self.online_eval_tp, 0).sum(axis=0)
        self.online_eval_fp = np.sum(self.online_eval_fp, 0).sum(axis=0)
        self.online_eval_fn = np.sum(self.online_eval_fn, 0).sum(axis=0)

        global_dc_per_class = [i for i in [2 * i / (2 * i + j + k + 1e-8) for i, j, k in
                                           zip(self.online_eval_tp, self.online_eval_fp, self.online_eval_fn)]
                               if not np.isnan(i)]
        # self.all_val_eval_metrics.append(np.mean(global_dc_per_class))
        self.all_val_eval_metrics.append(np.mean([kidney_dc_per, cancer_dc_per]))

        self.print_to_log_file("Val glob dc per class:",
                               str(global_dc_per_class))
        self.print_to_log_file("Val avg dc per class:",
                               )
        self.writer.add_scalars('train/Val_glob_dc_per_class', {'kidney': global_dc_per_class[0],
                                                                'cancer': global_dc_per_class[1]},
                                self.epoch)
        self.writer.add_scalars('train/Val_avg_dc_per_class', {'kidney': kidney_dc_per,
                                                               'cancer': cancer_dc_per},
                                self.epoch)

        if self.history_best < (0.4*kidney_dc_per + 0.6 * cancer_dc_per):
            self.history_best = (0.4*kidney_dc_per + 0.6 * cancer_dc_per)
            self.save_checkpoint(join(self.output_folder, "model_history_best.model"))
            print('save history best model...')
