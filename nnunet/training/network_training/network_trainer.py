from nnunet.unet_nested.utils import count_param
...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        self.train_loss_MA_eps = 3e-4
        self.save_every = 5
        self.save_latest_only = False
        self.max_num_epochs = 1000
        self.num_batches_per_epoch = 361
        ...
        self.writer = SummaryWriter(
            self.my_config.path, comment=self.my_config.comment)
        self.graph_flag = True
        self.print_para_flag = False
    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            my_keys = [str(i).zfill(5) for i in range(210)]
            random.shuffle(my_keys)
            tr_keys = my_keys[self.my_config.val_num:]
            val_keys = my_keys[:self.my_config.val_num]
        else:
    ...
    def plot_progress(self):
        ...
        try:
            ...
            ax.plot(x_values, self.all_tr_losses,
                    color='b', ls='-', label="loss_tr")

            ax.plot(x_values, self.all_val_losses, color='r',
                    ls='-', label="loss_val, train=False")

            if len(self.all_val_losses_tr_mode) > 0:
                ax.plot(x_values, self.all_val_losses_tr_mode,
                        color='g', ls='-', label="loss_val, train=True")
            if len(self.all_val_eval_metrics) == len(self.all_val_losses):
                ax2.plot(x_values, self.all_val_eval_metrics,
                         color='g', ls='--', label="evaluation metric")
    ...
    def run_training(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            for b in range(self.num_batches_per_epoch):
                ...
                if b != 0 and b % self.my_config.GA_size == 0:
                    self.optimizer.step()
            ...
            with torch.no_grad():
                ...
                self.print_to_log_file(
                    "val loss (train=False): %.4f" % self.all_val_losses[-1])

                self.writer.add_scalars('train/loss', {'train_loss': self.all_tr_losses[-1],
                ...
                if self.also_val_in_tr_mode:
                    ...
                    self.print_to_log_file(
                        "val loss (train=True): %.4f" % self.all_val_losses_tr_mode[-1])
                    self.writer.add_scalars('train/loss', {'train_loss': self.all_tr_losses[-1],
                                                           'val_loss': self.all_val_losses[-1]},
                                            global_step=self.epoch)
    ...
    def maybe_update_lr(self):
        ...
        if self.lr_scheduler is not None:
            assert isinstance(
                self.lr_scheduler, (lr_scheduler.ReduceLROnPlateau, lr_scheduler._LRScheduler))
    ...
    def maybe_save_checkpoint(self):
        ...
        if self.epoch % self.save_every == (self.save_every - 1):
            ...
            self.save_checkpoint(
                join(self.output_folder, "model_latest.model"))
    ...
    def manage_patience(self):
        ...
        if self.patience is not None:
            ...
            self.print_to_log_file(
                "current best_val_eval_criterion_MA is %.4f0" % self.best_val_eval_criterion_MA)
            self.print_to_log_file(
                "current val_eval_criterion_MA is %.4f" % self.val_eval_criterion_MA)
            self.writer.add_scalars('val/current', {'best_val_eval_criterion_MA': self.best_val_eval_criterion_MA,
            ...
            if self.train_loss_MA + self.train_loss_MA_eps < self.best_MA_tr_loss_for_patience:
                ...
                self.best_epoch_based_on_MA_tr_loss = self.epoch
                self.print_to_log_file(
                    "New best epoch (train loss MA): %03.4f" % self.best_MA_tr_loss_for_patience)
                self.writer.add_scalar('val/New_best_MA_tr_loss_for_patience', self.best_MA_tr_loss_for_patience,
                                       self.epoch)
            else:
                self.print_to_log_file("No improvement: current train MA %03.4f, best: %03.4f, eps is %03.4f" %
                                       (self.train_loss_MA, self.best_MA_tr_loss_for_patience, self.train_loss_MA_eps))
                self.writer.add_scalars('val/No_improvement', {'train_loss_MA': self.train_loss_MA,
            ...
            if self.epoch - self.best_epoch_based_on_MA_tr_loss > self.patience:
                if self.optimizer.param_groups[0]['lr'] > self.lr_threshold:
                    self.print_to_log_file(
                        "My patience ended, but I believe I need more time (lr > 1e-6)")
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if self.print_para_flag:
            param = count_param(self.network)
            y = self.network(data)
            print('Output shape:', y.shape)
            print('unet_nested totoal parameters: %.2fM (%d)' %
                  (param / 1e6, param))
            self.print_para_flag = False

        # if self.graph_flag:
        #     self.writer.add_graph(self.network, data)
        ...
        if do_backprop:
            ...
            # if self.num_batches_per_epoch / self.my_config.GA_size == 0:
            # self.optimizer.step()
            # self.optimizer.zero_grad()
    ...
    def find_lr(self, num_iters=1000, init_value=1e-6, final_value=10., beta=0.98):
        ...
        for batch_num in range(1, num_iters + 1):
            ...
            loss = self.run_iteration(
                self.tr_gen, do_backprop=True, run_online_evaluation=False).data.item() + 1
